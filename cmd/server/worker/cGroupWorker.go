...
type CGroupTask struct {
	...
	Func  func(Environment, pluginregistry.Container) error
}
...
func (cGT CGroupTask) Work() {

	failChan := make(chan failed, len(cGT.Group.Container))
	for _, container := range cGT.Group.Container {
		err := cGT.Func(cGT.Env, container)
		if err != nil {
			log.Warn(log.Fields{
				"package":   "main/worker",
				"function":  "Run",
	...
	for len(failChan) > 0 {
		...
		err := cGT.Func(cGT.Env, elem.container)
		if err != nil {
			log.Warn(log.Fields{
				"package":   "main/worker",
				"function":  "Run",
			...
			if elem.counter < cGT.Env.ErrorRetries {
				elem.counter++
